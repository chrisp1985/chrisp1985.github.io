(function() {
    ...
    [
        ...
        },
        {
            ...
            pythondriver:"This example shows the creation of a webdriver using Python bindings. There are some weird bits, like visibility of element requiring double parentheses around the locator and some of the syntax is slightly different elsewhere too, but generally if you can use the Java version it’s reasonably easy to get set up with Python. This specific class allows for the driver to be setup, so that when the module is imported and used in a different class, the method is available. But it also contains the ‘if __name__ == \"__main__\":’ statement at the bottom, which means it can be run in isolation (ie doesn’t need anything else to make the method execute). I’m using PyCharm (JetBrains) as an IDE, and when I execute the script it automatically passes the name as __main__, which it wouldn’t do if the module was imported.",
